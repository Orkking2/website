...
- Page lists, collection reading lists, and navigation sort newest-modified first (approved 2026-09-09); do not add explicit `order` frontmatter. Saved file times are preserved by content hash in `src/content/.modification-times.json`; commit this automatically maintained record with content changes. Git commit time is a fallback for committed content without a matching saved record. Writing collections inherit their newest child’s date, recursively, so editing an introduction or cover does not outrank a newer article (clarified 2026-09-09). Empty collections use their own index date; other directories inherit the newest time among their index and descendants. Equal times sort by title and route. These derived dates do not replace editorial publication/revision dates.
...
Photograph titles are embedded at the top-left of generated variants in the existing outlined watermark treatment, leaving source masters untouched. Captions appear below images, alongside a stable `/photography#photo-ID` link and any `.txt` essay link. Clicking opens the viewer and updates the URL; reload and sharing restore the image, while Back returns to the gallery. Keep a useful anchor and direct image link without JavaScript. Display images, including the direct fallback, are capped at 2400 pixels wide. As approved on 2026-09-08, an optional full-resolution, watermarked lossless WebP download is exempt from that cap. It is an ordinary link and never a responsive image candidate. New masters are full-resolution lossless WebP derived directly from the original into 8-bit sRGB; older JPEG masters are upgraded from their private originals when reviewed. Only ready masters are committed, alongside their records.
