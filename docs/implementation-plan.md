...
- The site remains non-indexable (`noindex, nofollow`, `robots.txt` disallow). It is deployed to a private, unlisted Cloudflare Pages preview for peer design-direction review — see D-008.
- The repository is now connected to GitHub at [`Orkking2/website`](https://github.com/Orkking2/website) (public), with `main` and `feat/identity-prototype` both pushed. Cloudflare Pages is not yet connected to it — see D-012 and "Deploying to Cloudflare Pages" in the README. This is still not the full Phase 7 production launch: no Cloudflare Pages Git connection, no custom domain, and indexing remains disabled.
...
- [x] Spike the smallest maintained metadata and image toolchain against the representative HEIC batch. The local intake step may be macOS-specific, but the committed publication masters and production build must remain reproducible in CI and Cloudflare Pages.
...
- [ ] Configure Cloudflare Pages with production branch `main`, build command `npm run build`, output directory `build`, and Node `22.23.2`.
