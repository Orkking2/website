...
| D-017 | 2026-09-05 | The Worker's first successful build confirmed (deployed to its `*.workers.dev` subdomain); Nicolas asked to attach `nebve.com`. Added `routes: [{ pattern: "nebve.com", custom_domain: true }]` to `wrangler.jsonc` so the next deploy attaches the domain automatically via Cloudflare's documented Wrangler-configuration method, rather than a manual dashboard step. This only succeeds if the deploying Cloudflare account owns `nebve.com`'s zone — D-008's unresolved account question is the most likely failure mode if it doesn't attach. `www` and the `*.workers.dev` subdomain remain undecided.                                                                                                                                                                                                                                                                                                     | Applied; live result (including whether the D-008 account question blocks it) not yet observed               |
...
- **2026-09-05:** The Worker deployed successfully to a `*.workers.dev` subdomain. Added a `custom_domain` route for `nebve.com` to `wrangler.jsonc` so the next deploy attaches it automatically (D-017); success depends on D-008's still-unresolved question of which Cloudflare account owns the zone.
