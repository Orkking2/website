...
- The site remains non-indexable (`noindex, nofollow`, `robots.txt` disallow) and has no Git remote. It is deployed to a private, unlisted Cloudflare Pages preview for peer design-direction review — see D-008. This is not the Phase 7 production launch: no GitHub connection, no custom domain, and indexing remains disabled.
