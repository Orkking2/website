...
Ordering is derived from saved file times; there is no `order` frontmatter field. Content checks, development, and builds maintain `src/content/.modification-times.json`, recording each page's content hash and modification time. Commit that file with the pages it describes. The matching hash preserves the save time after a bulk commit or a fresh checkout, so Git's shared commit timestamp does not flatten the article order. A real local edit records its new filesystem modification time; merely touching an unchanged file does not change its saved date. A committed page without a matching record falls back to its latest Git commit time, while new files and sources without Git use filesystem time.

Writing collections with children use their newest child's date, recursively: editing a collection introduction or cover does not move older articles above newer writing. Empty collections use their own index date. Other directories use the newest time among their index and descendants. Hidden files, photographs, and other non-page assets do not affect ordering. Equal timestamps sort by title, then route. Publication dates, `updated`, and `featured` do not override this order, and automatic modification times do not change the displayed first-publication or substantive-revision dates. Numbered lists derive their numbers from this sorted sequence. Photo-essay image sequences remain authored; the gallery still uses capture time.
